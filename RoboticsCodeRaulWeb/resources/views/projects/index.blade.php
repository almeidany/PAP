                                        ...
                                        <tr>
                                            <td class="px-0">
                                                <div class="d-flex align-items-center justify-content-center">
                                                    @if($project->photo)  <!-- Verifique se o campo se chama 'photo' ou 'foto' -->
                                                    ...
                                                    <div class="ms-3">
                                                        ...
                                                        <small class="text-muted">{{ \Carbon\Carbon::parse($project->start_date)->format('d/m/Y') }}</small>
                                                    </div>
                                                </div>
                                            </td>
                                            <td class="px-0">{{ $project->category }}</td> <!-- Exibe a categoria como sigla -->
                                            <td class="px-0">
                                                <a href="{{ $project->github_url }}" target="_blank" class="text-primary text-decoration-underline">
                                                {{ $project->github_url }}
                                                </a>
                                            </td>
                                            <td class="px-0 text-dark font-weight-medium">
                                                @if($project->end_date)
                                                    ...
                                                    <small class="text-muted d-block">{{ \Carbon\Carbon::parse($project->end_date)->format('d/m/Y') }}</small>
                                                @else
                                                    <span class="badge bg-warning">Em Progresso</span>
                                                @endif
                                            </td>
                                            <td class="px-0">
                                                <form id="deleteProject{{ $project->id }}" action="{{ route('projects.destroy', $project->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
